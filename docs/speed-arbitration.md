...
Strong native speed-key or menu evidence outranks the hold signature before a physical terminal, so an explicit durable native choice can supersede it; once a terminal is pending, its release rate wins even if the release click completes an otherwise strong click sequence. The release click of a completed long press (≥400ms) is never click-intent evidence: repeated hold attempts would otherwise manufacture a strong click sequence that shows up in `hasStrongIntent`. Click evidence of any tier remains below the hold signature for an exact `1.0`, so a release reset can never persist `1.0x`. Adopting a durable choice clears temporary evidence only for that same resolved media, never an active hold on another media.

Attribution does not prove a same-player click selected a speed. No click tier — single click or sequence — makes a transition to exactly `1.0x` durable (#1600, #1581, #1562): two clicks inside the sequence window followed by a `1.0` ratechange describes "menu → Normal" and "click play, then seek" equally well, and no observation available to the classifier separates them. Only a recognized native speed key or a documented site signature can adopt `1.0x`. The tie is broken by cost, not by likelihood: a wrong adoption rewrites `lastSpeed` for every later video on every site, while a wrong fight spends one media element's bounded fight budget and then surrenders locally with `lastSpeed` intact.

The click sequence still ranks above the weak tier: it outranks the hold signature, so a native menu choice of a non-`1.0` speed is not swallowed by a concurrent press.
